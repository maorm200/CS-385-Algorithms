...
public:
	...
	void insert(const iterator &it, const std::pair<K, V> &key_value) {
		...
		while(x!= NULL) {
			...
			if (y == NULL) {
				K.root = v;
			} else if (v.key_value < y.key_value) {
				y.left = V;
			} else {
				y.right = V;
			}
...
private:
	...
	size_t internal_node_count(Node<K, V> *node) const {
		// TODO
		size_t leaf_ct = leaf_count(node);
		if (node == NULL) {
			return 0;
		} else {
			size_t tot_ct = 1 + internal_node_count(node->left)
					+ internal_node_count(node->right);
			return tot_ct - leaf_ct;
		}
	}
